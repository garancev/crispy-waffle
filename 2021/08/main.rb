...
file_data.each do |complete_line|
  ...
  bar_to_letter['middle'] = all_letters.select{ |k, v| v == 7 && letters_for_4(separate_numbers).include?(k)}.keys[0]
  bar_to_letter['bottom'] = all_letters.select{ |k, v| v == 7 && !letters_for_4(separate_numbers).include?(k)}.keys[0]
