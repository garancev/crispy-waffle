...
def count_visited_positions(motions, head, tails)
  motions.each_with_index do |motion, index|
    times = motion[1].to_i
    log = motion == ["L", "8"]
    # puts head.inspect if log
    # puts tails.inspect if log
    # puts "== #{motion[0]} #{motion[1]} =="
    while times > 0 do
      # visualise_progress(head[:positions].last, tails)  if log
      # puts "head goes left 1 from #{head[:positions].last.inspect}" if log
      head[:positions] << move_head(head[:positions].last, motion)
      tails.each_with_index do |tail, index|
        previous = index == 0 ? head : tails[index - 1]
        # puts "Previous knot is at #{previous[:positions].last.inspect}" if log
        tail[:positions] << move_tail(tail[:positions].last, previous[:positions].last)
        # puts "knot #{index + 1} moved to #{tail[:positions].last.inspect} (from #{tail[:positions][tail[:positions].size - 2].inspect})" if log  && times == 1
      end
      times -= 1
    end
  end
...
puts count_visited_positions_2_knots(file_data)

puts "~~~ and how many positions does the tail of the rope visit at least once? ~~~"
puts count_visited_positions_10_knots(file_data)
puts "~~~ and with the larger example? ~~~"
# puts count_visited_positions_10_knots(file_data_2)